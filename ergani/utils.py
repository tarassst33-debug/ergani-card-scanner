...
def parse_workcard_movement_type(value: Any) -> Optional[WorkCardMovementType]:
    """Map Ergani API movement codes/text to ARRIVAL or DEPARTURE."""
    if value is None or value == "":
        return None
    text = str(value).strip().upper()
    if text in {"0", "ARRIVAL", "ΠΡΟΣΕΛΕΥΣΗ", "PROSELYSIS"}:
        return "ARRIVAL"
    if text in {"1", "DEPARTURE", "ΑΠΟΧΩΡΗΣΗ", "APOCHORISI"}:
        return "DEPARTURE"
    return None
